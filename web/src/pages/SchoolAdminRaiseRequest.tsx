    ...
    mutationFn: async () => {
      ...
      const uploadAndGetUrl = async () => {
        if (attachmentFile && !attachmentUrl) {
          const res = await schoolAdminQueriesService.uploadFile(attachmentFile);
          return res.url;
        }
      ...
      if (selectedType === SchoolAdminQueryType.CUSTOM_MESSAGE) {
        ...
        if (recipient === 'super_admin') return schoolAdminQueriesService.sendRequest(data);
        if (recipient === 'category_admin') return schoolAdminQueriesService.sendToCategoryAdmin(data);
        return schoolAdminQueriesService.sendToSubCategoryAdmin(data);
      }
