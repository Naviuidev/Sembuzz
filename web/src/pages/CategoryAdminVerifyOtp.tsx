...
export const CategoryAdminVerifyOtp = () => {
  ...
  useEffect(() => {
    if (!email) {
      navigate('/category-admin/forgot-password', { replace: true });
    }
  }, [email, navigate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/category-admin/auth/forgot-password/verify-otp', { email, otp });
      setSuccess('OTP verified successfully!');
      setTimeout(() => {
        navigate('/category-admin/forgot-password/reset', { state: { email, otp } });
      }, 1000);
    } catch (err: unknown) {
      const errorMessage = (err as { response?: { data?: { message?: string }; message?: string }; message?: string })?.response?.data?.message ?? (err as { message?: string })?.message ?? 'Invalid OTP. Please try again.';
        ...
        <div style={{ width: '100%', maxWidth: '90%' }}>
          ...
          {devOtp && (
            <div className="alert alert-info mb-3" role="alert" style={{ borderRadius: '8px' }}>
              <strong>Development:</strong> Use this OTP: <code style={{ fontSize: '1.25rem', letterSpacing: '0.2em' }}>{devOtp}</code>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="text-center mt-3">
              <a href="/category-admin/forgot-password" style={{ color: '#1a1f2e', textDecoration: 'none', fontSize: '0.9rem' }}>
                Back
              </a>
            </div>
          </form>
