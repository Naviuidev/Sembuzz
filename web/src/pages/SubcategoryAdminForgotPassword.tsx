...
export const SubcategoryAdminForgotPassword = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post('/subcategory-admin/auth/forgot-password/request-otp', { email: email.trim() });
      setSuccess(`OTP has been sent to ${response.data.email}`);
      setTimeout(() => {
        navigate('/subcategory-admin/forgot-password/verify-otp', { state: { email: email.trim() } });
      }, 2000);
    } catch (err: unknown) {
      const errorMessage = (err as { response?: { data?: { message?: string }; message?: string }; message?: string })
        ?.response?.data?.message ?? (err as { message?: string })?.message ?? 'Failed to send OTP. Please try again.';
