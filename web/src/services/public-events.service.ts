...
export const publicEventsService = {
  /** Approved events with status scheduled (awaiting publishAt). */
  getScheduled: async (schoolId: string): Promise<ApprovedEventPublic[]> => {
  ...
  getUpcomingByDate: async (date: string, schoolId?: string | null): Promise<UpcomingPostPublic[]> => {
