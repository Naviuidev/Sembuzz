...
export default function LikedNewsScreen() {
  ...
  const [selected, setSelected] = useState<LikedEventItem | null>(null);

  const load = useCallback(async () => {
    try {
      const list = await userEventsService.getLikedEvents();
      setItems(Array.isArray(list) ? list : []);
      setError(null);
    } catch {
      setItems([]);
      setError('Could not load liked news. Pull to try again.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>
              No liked news yet. Like posts from your home feed to see them here.
            </Text>
          }
        />
      )}
