...
export const userEventsService = {
  ...
  toggleLike: async (eventId: string): Promise<{ liked: boolean; count: number }> => {
    const response = await api.post<{ liked: boolean; count: number }>(`/user/events/${eventId}/like`);
    return response.data;
  },

  getComments: async (eventId: string): Promise<EventCommentResponse[]> => {
    const response = await api.get<EventCommentResponse[]>(`/user/events/${eventId}/comments`);
    return response.data;
  },
  ...
  addComment: async (
    ...
    const response = await api.post<{ comment: EventCommentResponse; commentCount: number }>(
