...
- **Super-admin login returns 500 (POST /super-admin/auth/login):** (1) Run `pm2 logs sembuzz-api` and try logging in again; the log will show the real error (e.g. "Table 'super_admins' doesn't exist" or Prisma/JWT error). (2) If the `super_admins` table is missing, run `npx prisma migrate deploy` (migration `20260120000000_create_super_admins` creates it with IF NOT EXISTS). (3) Ensure at least one super-admin user exists: run `npm run seed:super-admin` (or insert manually in phpMyAdmin using a bcrypt hash for the password). (4) Ensure backend `.env` has `JWT_SECRET` set (e.g. `openssl rand -base64 32`) and restart the API (`pm2 restart sembuzz-api`).
- **GET /super-admin/features returns 500:** (1) Run `pm2 logs sembuzz-api` and reload the super-admin page; the log will show the real error (e.g. "Table 'features' doesn't exist"). (2) Run `npx prisma migrate deploy` (migration `20260121000000_create_features_tables` creates `features` and `school_features` with IF NOT EXISTS). (3) Seed features so the list is not empty: `npm run seed:super-admin` or `npm run prisma:seed` / run `backend/prisma/seed-features.sql` or the features seed script. (4) Restart the API (`pm2 restart sembuzz-api`).
- **POST /super-admin/schools returns 500 (create school):** (1) Run `pm2 logs sembuzz-api`, try creating a school again, and check the logged error. (2) If the log shows "Unknown column" or missing columns on `schools`, run `npx prisma migrate deploy` (migration `20260122000000_add_schools_extra_columns` adds country, state, tenure, domain, image). If that migration fails with "Duplicate column name", the columns already exist — run `npx prisma migrate resolve --applied 20260122000000_add_schools_extra_columns`. (3) If the log mentions `ads_admins`, ensure migrations are applied (`npx prisma migrate deploy`). (4) Restart the API (`pm2 restart sembuzz-api`).
