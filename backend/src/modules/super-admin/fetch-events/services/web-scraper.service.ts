...
const USER_AGENT =
  'Mozilla/5.0 (compatible; SembuzzEventsBot/1.0; +https://sembuzz.com/bot)';
...
export class WebScraperService {
  ...
  private readonly maxDetailPrefetch: number;
  private readonly crawlFetchParallel: number;

  constructor(
    private readonly config: ConfigService,
    private readonly playwright: PlaywrightRendererService,
    private readonly eventCandidates: EventCandidateExtractorService,
  ) {
  ...
  async fetchAndClean(url: string): Promise<ScrapedPage> {
   ...
   */
  async crawlFromSeed(seedUrl: string): Promise<ScrapedPage[]> {
    const seedNorm = this.normalizeUrl(seedUrl);
    if (!seedNorm) return [await this.fetchAndClean(seedUrl)];
    ...
    const enqueue = (raw: string) => {
      ...
      if (u.hostname !== seedParsed.hostname) return;
    ...
    while (queue.length > 0 && pages.length < this.crawlMaxPages) {
      ...
      if (batchNorms.length === 0) break;

      const htmlResults = await Promise.all(
        batchNorms.map(async (norm) => {
          try {
            return { norm, html: await this.fetchHtml(norm) };
          } catch (e) {
            this.logger.warn(`Crawl skip ${norm}: ${(e as Error).message}`);
            return { norm, html: null as string | null };
          }
        }),
      );
      ...
      for (const { norm, html } of htmlResults) {
        ...
        if (norm === seedNorm && this.isLiveWhaleCalendarHtml(html)) {
          ...
          if (fromJson) {
            this.logger.log(
              `LiveWhale JSON feed for ${seedParsed.hostname}: ${fromJson.cleanedText.length} chars, ${fromJson.detailLinks.length} detail link(s)`,
            );
            page = fromJson;
        ...
        for (const d of page.detailLinks) {
          if (detailPrefetchRemaining <= 0) break;
          if (!this.isLikelyEventDetailUrl(d, seedParsed)) continue;
          const dn = this.normalizeUrl(d);
          if (!dn || dn === norm) continue;
  ...
  private isLiveWhaleEventRow(v: unknown): v is Record<string, unknown> {
    return v !== null && typeof v === 'object' && typeof (v as Record<string, unknown>).title === 'string';
  }
  ...
  private isLiveWhaleCalendarHtml(html: string): boolean {
    if (!/LiveWhale/i.test(html)) return false;
    if (/<meta[^>]+name\s*=\s*["']generator["'][^>]*content\s*=\s*["'][^"']*LiveWhale/i.test(html)) {
      return true;
    }
    if (/LiveWhale\s+Calendar/i.test(html)) return true;
    if (/\/live\/resource\/js\/[^"']*livewhale/i.test(html.toLowerCase())) return true;
    return false;
  }
   ...
   */
  private async tryBuildPageFromLiveWhaleJson(origin: string, seedNorm: string): Promise<ScrapedPage | null> {
    ...
    if (!this.isLiveWhaleEventRow(data[0])) return null;
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const types = Array.isArray(ev.event_types)
        ? (ev.event_types as unknown[]).filter((t): t is string => typeof t === 'string').join(', ')
        : '';
      const tags = Array.isArray(ev.tags)
        ? (ev.tags as unknown[]).filter((t): t is string => typeof t === 'string').join(', ')
        : '';

      lines.push('');
      lines.push(`--- Event ${i + 1} ---`);
      lines.push(`Title: ${title}`);
      if (dateIso) lines.push(`Start (ISO): ${dateIso}`);
      if (date2Iso) lines.push(`End (ISO): ${date2Iso}`);
      if (dateUtc && !dateIso) lines.push(`Start (UTC field): ${dateUtc}`);
      if (dateLabel) lines.push(`Date (label): ${dateLabel}`);
      if (timeLabel) lines.push(`Time (label): ${timeLabel}`);
      if (loc) lines.push(`Location: ${loc}`);
      if (url) lines.push(`Detail URL: ${url}`);
      if (desc) lines.push(`Description: ${desc}`);
      if (contact) lines.push(`Contact: ${contact}`);
      if (types) lines.push(`Categories: ${types}`);
      if (tags) lines.push(`Tags: ${tags}`);
    }

    const cleanedText = lines.join('\n');
    if (cleanedText.length < 400) return null;
  ...
  private async fetchHtmlStatic(url: string): Promise<string> {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), SCRAPE_TIMEOUT_MS);
    try {
  ...
  private isLikelyEventDetailUrl(abs: string, seed: URL): boolean {
    let u: URL;
    try {
      u = new URL(abs);
    } catch {
    ...
    if (/[?&](event_id|eventid|eid|evt)=/i.test(q)) return true;
    if (/[?&]event=\d+/i.test(q)) return true;
    if (/(^|\/)(event|events)\/[^/]{4,}\/?$/i.test(path)) return true;
    if (/(^|\/)(event|events)\/[^/]+\/[^/]{3,}\/?$/i.test(path)) return true;
    if (/\/(workshop|seminar|symposium|lecture|colloquium)\/[^/]{4,}\/?$/i.test(path)) return true;
    return false;
  }
  ...
  private extractMainSection(html: string): string {
    ...
    for (const re of candidates) {
      ...
      if (m && m[0].length > 600) return cap(m[0], 600_000);
    }
  ...
  private extractImageUrls(html: string, baseUrl: string): string[] {
    ...
    while ((m = re.exec(html))) {
      ...
      if (abs && this.looksLikeContentImage(abs)) out.add(abs);
      if (out.size >= 40) break;
    }
    return Array.from(out);
  }

  private extractDetailLinks(html: string, baseUrl: string): string[] {
    const out = new Set<string>();
    const re = /<a\b[^>]*\bhref=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi;
    let m: RegExpExecArray | null;
    let iterations = 0;
    while ((m = re.exec(html))) {
      if (++iterations > MAX_ANCHOR_SCAN_ITERATIONS) break;
      const href = m[1];
  ...
  private looksLikeContentImage(src: string): boolean {
    const lower = src.toLowerCase();
    if (lower.endsWith('.svg')) return false;
    if (lower.includes('icon') || lower.includes('logo') || lower.includes('avatar')) return false;
    if (lower.includes('sprite') || lower.includes('placeholder')) return false;
   ...
   */
  private htmlSliceForLinkScan(html: string): string {
    if (html.length <= MAX_HTML_CHARS_LINK_SCAN) return html;
   ...
   */
  private discoverCrawlTargets(currentUrl: string, html: string, seed: URL): string[] {
    const scan = this.htmlSliceForLinkScan(html);
    const out = new Set<string>();
    const current = new URL(currentUrl);

    const relNext =
      scan.match(/<link[^>]*\brel\s*=\s*["'][^"']*next[^"']*["'][^>]*\bhref\s*=\s*["']([^"']+)["']/i) ||
      scan.match(/<link[^>]*\bhref\s*=\s*["']([^"']+)["'][^>]*\brel\s*=\s*["'][^"']*next[^"']*["']/i);
    if (relNext?.[1]) {
      const abs = this.toAbsolute(relNext[1], currentUrl);
      if (abs && this.crawlLinkScore(seed, current, new URL(abs)) >= 8) out.add(abs);
    }
  ...
  private crawlLinkScore(seed: URL, current: URL, link: URL): number {
    ...
    if (/mailto:|tel:/i.test(link.href)) return -999;
    ...
    if (/[?&]page=\d+/i.test(search)) s += 15;

    // Path-based paging: /events/page/2/, /listing/pg/3
    if (/(^|\/)(page|pg|pages)\/\d{1,4}(\/|$|\?)/i.test(path)) s += 44;

    // Calendar / archive shapes
    if (/\/\d{4}(-|\/)\d{2}(-|\/)\d{0,2}\b/.test(path)) s += 22;
    if (/[?&](month|year|m|mo|cal|view|mode|w|week|day)=/i.test(search)) s += 22;
    if (/ical|\.ics(\?|$)/i.test(full)) s -= 50;
    ...
    if (/category|categories|topics?|types?|tags?\/|\/tag\//i.test(full)) s += 22;
    ...
    if (seedPath !== '/' && seedPath !== '') {
      if (path === seedPath || path.startsWith(`${seedPath}/`)) s += 18;
    }
   ...
   */
  private nonEventPathPenalty(path: string, search: string): number {
    const full = `${path} ${search}`;
    if (/events?|calendar|agenda|schedule|symposium|seminar|workshop|colloquium|happening|register|rsvp/i.test(full)) {
      return 0;
    }
    if (/(^|\/)(admissions?|financial-aid|tuition|apply|application)(?:\/|$)/i.test(path)) return -140;
    if (
      /(^|\/)(blogs?|faculty|staff\/|people\/|directory|careers|jobs|giving|donate|alumni|magazine|press-office|press\/)(?:\/|$)/i.test(
        path,
      )
    ) {
      return -140;
    }
    if (/\/news(\/|$)/i.test(path)) return -110;
    if (/(^|\/)(policy|policies|legal|veterans|disability-services)(?:\/|$)/i.test(path)) return -120;
    return 0;
  }
}
