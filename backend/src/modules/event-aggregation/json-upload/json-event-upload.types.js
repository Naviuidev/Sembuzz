...
exports.parseYmdToDate = parseYmdToDate;
exports.normalizeJsonUploadEvent = normalizeJsonUploadEvent;
...
function normalizeJsonUploadEvent(raw) {
    const title = pickString(raw, ['event_name', 'title', 'name', 'eventTitle']);
    if (!title)
        return null;
    const universityName = pickString(raw, ['university', 'school', 'schoolName', 'universityName']) ?? 'University';
    const calendarUrl = pickString(raw, ['calendar_url', 'calendarUrl', 'url']) ??
...
function groupKey(universityName, calendarUrl) {
    return `${universityName.trim().toLowerCase()}|${calendarUrl.trim().toLowerCase()}`;
}
//# sourceMappingURL=json-event-upload.types.js.map