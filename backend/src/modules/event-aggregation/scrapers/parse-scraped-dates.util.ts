...
export function parseScrapedDateLine(raw: string, zone: string): { start: Date | null; end: Date | null } {
  ...
  const rangeSameDay = s.match(
    /^(.+?\d{4})\s+(\d{1,2}:\d{2}\s*(?:am|pm))\s+to\s+(\d{1,2}:\d{2}\s*(?:am|pm))$/i,
  );
  if (rangeSameDay) {
  ...
  if (single) return { start: single.toJSDate(), end: null };

  return { start: null, end: null };
}

function tryLuxonParse(text: string, zone: string): DateTime | null {
  const t = text.trim();
  const formats = [
...
export const parseLocalistDateLine = parseScrapedDateLine;
